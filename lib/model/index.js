...
const { toArray, handleParam } = require('../util')
...
function Model (tableName, schema = {}, exInfo = {}) {
    ...
    setTimeout(() => {
      ...
      switch (this.type) {
        ...
        case 'select':
          ...
          // where select params must be a object
          if (this.conditions['select'] && Object.keys(this.conditions['select']).length) {
            let linkMode = 'and'
            const conditions = []
  ...
  self.create = obj => {
    if (handleParam(obj)) {
      ...
      throw Error(`create function mast have params`)
    }
    return self
  }
  // delete
  self.delete = (obj = {}) => {
    if (handleParam(obj)) {
      ...
      throw Error(`delete function mast have parmas`)
    }
    return self
  }
  // update
  self.update = (obj = {}, whereObj = {}) => {
    if (handleParam(obj)) {
      ...
      throw Error(`update function mast have parmas`)
    }
    return self
  }
  // select
  ...
  self.like = (obj, likeMode = 'LIKE', linkMode = 'AND') => {
    if (handleParam(obj)) {
      ...
      throw Error(`like function mast have parmas`)
    }
    return self
  }
  // limit
